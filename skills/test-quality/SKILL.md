---
name: test-quality
description: Write high-quality JUnit 5 tests with AssertJ assertions according to project conventions. Use when user says "add tests", "write tests", "improve test coverage", or when reviewing/creating test classes for Java code.
---
...
The suffix (and env profile) of a test class will effect how and when tests are being run. IT and Tests are part of the deployment pipeline. 

- ***IT**
    - **Description**: General integration test with the purpose to test the integration as a whole, meaning the tests go layers deep in the service, e.g. from resource (api endpoints) to database or other external integrations.
    - **Run**: Manually, Staging, Production, PCI
- ***Tests**
    - **Description**: Unit test with the specific purpose of testing a single method, validation or separate 
    - **Run**: Manually, Staging, Production, PCI
- ***Verify**
    - **Description**: Special cases for verifying integration against external integrations etc. 
    - **Run**: Manually
- ***Scripts**
    - **Description**: Used for manually applying changes for specific use-cases
    - **Run**: Manually

```java
// Class under test: PluginManager
PluginManagerTest           // ✅ Unit tests
PluginManagerIT             // ✅ Integration tests
